...
func Test_DotU8(t *testing.T) {
	...
	testCases := []TestCase{
		{
			Name: "vanilla",
			X:    []uint8{15, 6, 1, 12, 13, 9, 15, 18, 14, 6, 17, 16, 1, 3, 3, 9, 5, 13, 6, 20, 9, 2, 18, 13, 10, 1, 7, 10, 8, 7, 11, 18, 5, 19, 17, 19, 15, 0, 17, 18},
			Y:    []uint8{9, 2, 18, 13, 10, 1, 7, 10, 8, 7, 11, 18, 5, 19, 17, 19, 15, 0, 17, 18, 15, 6, 1, 12, 13, 9, 15, 18, 14, 6, 17, 16, 1, 3, 3, 9, 5, 13, 6, 20},
			Want: 4390,
			F:    dot[uint8, uint8],
			Skip: false,
		},
	...
	for _, tc := range testCases {
		...
		assert.Equal(t, tc.Want, tc.F(tc.X, tc.Y))
		assert.InEpsilon(t, tc.Want, tc.F(tc.X, tc.Y), 0.0001)
	}
}
