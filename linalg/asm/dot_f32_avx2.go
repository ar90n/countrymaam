//go:generate go run ./dot_f32_avx2.go -out dot_f32_avx2.s -stubs dot_f32_stub_avx2.go
//go:build ignore
// +build ignore
...
func main() {
	...
	acc := make([]VecVirtual, unroll_dot_f32_avx2)
	for i := 0; i < unroll_dot_f32_avx2; i++ {
		acc[i] = YMM()
	}
	...
	blockitems := 8 * unroll_dot_f32_avx2
	blocksize := 4 * blockitems
	Label("blockloop")
	CMPQ(n, U32(blockitems))
	JL(LabelRef("tail"))

	xs := make([]VecVirtual, unroll_dot_f32_avx2)
	ys := make([]VecVirtual, unroll_dot_f32_avx2)
	for i := 0; i < unroll_dot_f32_avx2; i++ {
		xs[i] = YMM()
		ys[i] = YMM()
	}
	...
	for i := 0; i < unroll_dot_f32_avx2; i++ {
		...
		VMOVUPS(y.Offset(32*i), ys[i])
		VFMADD231PS(ys[i], xs[i], acc[i])
	}
	...
	Label("reduce")
	for i := 1; i < unroll_dot_f32_avx2; i++ {
